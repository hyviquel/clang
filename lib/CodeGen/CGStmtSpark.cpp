...
void CodeGenFunction::EmitSparkMapping(llvm::raw_fd_ostream &SPARK_FILE,
  ...
  for (auto it = info.CounterInfo.begin(); it != info.CounterInfo.end(); ++it) {
    ...
    SPARK_FILE
        << "    val blockSize_" << MappingId << "_" << NbIndex
        << " = if(info.schedulingSize <= 0) ((bound_" << MappingId << "_"
        << NbIndex
        << ").toFloat/_parallelism).floor.toLong else info.schedulingSize\n";
    ...
    SPARK_FILE << " // List of Index " << VarCnt->getName() << "\n";
    ...
    SPARK_FILE << " // Dataset of Index " << VarCnt->getName() << "\n";
